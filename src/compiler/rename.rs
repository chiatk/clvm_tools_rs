    ...
    match body.borrow() {
        SExp::Atom(l,name) => {
            match namemap.get(name) {
                Some(v) => { return Rc::new(SExp::Atom(l.clone(), v.to_vec())); },
                None => { return body; }
            }
        },
        SExp::Cons(l,f,r) => {
            match f.borrow() {
                SExp::Atom(la,q) => {
                    if *q == "q".as_bytes().to_vec() {
                        ...
                        return r.proper_list().map(|x| {
                            match &x[..] {
                                [v] => {
                                    return Rc::new(SExp::Cons(
                                        l.clone(),
                                        Rc::new(SExp::atom_from_string(la.clone(), &"quote".to_string())),
                                        Rc::new(SExp::Cons(v.loc(),Rc::new(v.clone()),Rc::new(SExp::Nil(v.loc()))))
                                    ));
                                },
                                _ => { return body.clone(); }
                            }
...
fn rename_in_bodyform(
    ...
    match b.borrow() {
        ...
        BodyForm::Quoted(atom) => {
            match atom.borrow() {
                SExp::Atom(l,n) => {
                    match namemap.get(n) {
                        Some(named) => {
                            BodyForm::Quoted(SExp::Atom(l.clone(), named.to_vec()))
                        },
        ...
        BodyForm::Value(atom) => {
            match atom.borrow() {
                SExp::Atom(l,n) => {
                    match namemap.get(n) {
                        Some(named) => {
                            BodyForm::Value(SExp::Atom(l.clone(), named.to_vec()))
                        },
        ...
        BodyForm::Call(l,vs) => {
            let new_vs = vs.iter().map(|x| Rc::new(rename_in_bodyform(namemap, x.clone()))).collect();
...
fn rename_args_bodyform(b: &BodyForm) -> BodyForm {
    match b.borrow() {
        BodyForm::Let(l,bindings,body) => {
            let renames: Vec<(Vec<u8>, Binding)> =
                bindings.iter().map(|x| make_binding_unique(x.borrow())).collect();
        ...
        BodyForm::Call(l,vs) => {
            let new_vs = vs.iter().map(|a| Rc::new(rename_args_bodyform(a))).collect();
            BodyForm::Call(l.clone(), new_vs)
        }
    }
}
...
fn rename_in_helperform(
    ...
    match h {
        HelperForm::Defconstant(l,n,body) => {
            HelperForm::Defconstant(l.clone(),n.to_vec(),Rc::new(rename_in_bodyform(&namemap, body.clone())))
        },
        HelperForm::Defmacro(l,n,arg,body) => {
            HelperForm::Defmacro(l.clone(),n.to_vec(),arg.clone(),Rc::new(rename_in_compileform(&namemap, body.clone())))
        },
        HelperForm::Defun(l,n,inline,arg,body) => {
            HelperForm::Defun(l.clone(),n.to_vec(),*inline,arg.clone(),Rc::new(rename_in_bodyform(&namemap, body.clone())))
        }
    }
}
    ...
    match h {
        ...
        HelperForm::Defmacro(l,n,arg,body) => {
            ...
            HelperForm::Defmacro(
                ...
                n.clone(),
                local_renamed_arg,
                Rc::new(rename_in_compileform(&local_namemap, Rc::new(local_renamed_body)))
            )
        },
        HelperForm::Defun(l,n,inline,arg,body) => {
            ...
            HelperForm::Defun(
                ...
                *inline,
                local_renamed_arg,
                Rc::new(rename_in_bodyform(&local_namemap, Rc::new(local_renamed_body)))
            )
        }
    }
}
...
fn rename_in_compileform(
    ...
    CompileForm {
        loc: c.loc.clone(),
        args: c.args.clone(),
        helpers: c.helpers.iter().map(|x| rename_in_helperform(namemap, x)).collect(),
...
pub fn rename_children_compileform(c: &CompileForm) -> CompileForm {
    let local_renamed_helpers =
        c.helpers.iter().map(|x| rename_args_helperform(x)).collect();
...
pub fn rename_args_compileform(c: &CompileForm) -> CompileForm {
    ...
    CompileForm {
        ...
        helpers: local_renamed_helpers.iter().
            map(|x| rename_in_helperform(&local_namemap, x)).collect(),
        exp: Rc::new(rename_in_bodyform(&local_namemap, Rc::new(local_renamed_body)))
    }
}
